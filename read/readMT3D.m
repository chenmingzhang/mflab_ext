function COut=readMT3D(fname,userPeriods,userTSteps,userLays,userRows,userCols)
%READMT3D reads unformatted MT3DMS concentration file into struct 
%
% Example:
%   C=readMT3D([-]fname [,userPeriods [,userLays [,userRows [,userCols]]]]]])
...
doAllTSteps  = ~exist('userTSTeps','var')   || isempty(userTSteps);
doAllLayers  = ~exist('userLays' ,'var')    || isempty(userLays);
doAll        = doAllPeriods && doAllLayers;
...
if ~exist('userRows','var')    || isempty(userRows),    userRows=1:NROW; end
if ~exist('userCols','var')    || isempty(userCols),    userCols=1:NCOL; end
if ~exist('userPeriods','var') || isempty(userPeriods), userPeriods=1:NPER; end
if ~exist('userTSteps','var')  || isempty(userTSteps),  userTSteps=1:NSTP; end
