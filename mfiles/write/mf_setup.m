...
elseif numel(igrid)==1 % if there is only one, use it
    ...
    for i=2:length(igrid), s=[s ', ' mf_variables(igrid(i)).name]; end  %#ok
...
%% xlsread mac is different from xlsread windows !!!asdf
...
if isunix   %  size(Nnum,2)==size(Ntxt,2)
...
nam.LegalPCKG=cell(length(nam.MODEL),1);
...
if exist('LAYCBD','var')
    ...
    warning('mf_setup:LAYCBD:LgnoreSpreadSheet',...
        'mf_setup: LAYCBD found in workspace, ignoring LAYCBD in worksheet LAY.\n');
end
...
if bcf.SCEN    
    ...
    nNonc = length(Inonc); % number of non convertible layers in LAYCON
    ...
    if exist('TRAN','var')
        ...
        if GRID.AXIAL
            bcf.TRAN = TWOPIR(:,:,1:size(TRAN,3)).*TRAN;
        else
            bcf.TRAN = TRAN;
        end
    end
    ...
    % Then HY
    if exist('HY'  ,'var')
            % chenming this is to prevent that the domain is just a row or
            % a column a model necessary for tutorial Q1
        if isvector(HY) && gr.Nx~=1 && gr.Ny~=1,
            % the original purpose for this script is to let each value 
            % in a vector represent HY in a layer, however, this clashes
            % with one row and one column problem here
...
if lpf.SCEN || upw.SCEN
    ...
    if any(lpf.CHANI<=0)
        try
            ...
            if ndims(HANI)<3 && GRID.Nlay>1, % if not make it 3D
                lpf.HANI=XS(HANI(:));
            end
        catch ME
...
%  commented out by chenming TO 14-10-13. reason: NEVTOP can be 3.
    evt.NEVTOP =max(min(evt.NEVTOP,3),1);
%    evt.NEVTOP =max(min(evt.NEVTOP,2),1);
    ...
    if ets_SCEN  % extra for ets the columns PXDP and PETM
...
for ibc=1:length(activeStresses)  % for each BCN type one loop
...
if pcg.SCEN
    ...
    pcg.MXITER=MFLOWparvals(strmatchi('MXITER',MFLOWparnams,'exact'),pcg.SCEN);
    pcg.ITER1 =MFLOWparvals(strmatchi('ITER1' ,MFLOWparnams,'exact'),pcg.SCEN);
    pcg.NPCOND=MFLOWparvals(strmatchi('NPCOND',MFLOWparnams,'exact'),pcg.SCEN);
    pcg.HCLOSE=MFLOWparvals(strmatchi('HCLOSE',MFLOWparnams,'exact'),pcg.SCEN);
    pcg.RCLOSE=MFLOWparvals(strmatchi('RCLOSE',MFLOWparnams,'exact'),pcg.SCEN);
    pcg.RELAX =MFLOWparvals(strmatchi('RELAX' ,MFLOWparnams,'exact'),pcg.SCEN);
    pcg.NBPOL =MFLOWparvals(strmatchi('NBPOL' ,MFLOWparnams,'exact'),pcg.SCEN);
    pcg.IPRPCG=MFLOWparvals(strmatchi('IPRPCG',MFLOWparnams,'exact'),pcg.SCEN);
    pcg.MUTPCG=MFLOWparvals(strmatchi('MUTPCG',MFLOWparnams,'exact'),pcg.SCEN);
    pcg.DAMP  =MFLOWparvals(strmatchi('DAMP'  ,MFLOWparnams,'exact'),pcg.SCEN);
