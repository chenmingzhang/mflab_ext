...
%
% USAGE:
%    well       = animateObj.concXZ([gr,]species,ILay,well,varargin);
%
% gr       = gridObj
% species  = index/Nr of compoments/Species to plot
%            Names of components may be used instead, but must be given as
...
[o.gr,varargin] = getProp(varargin,'gridObj',o.gr);

i = strmatchi('gridObj',cellfun(@class,varargin,'UniformOutput',false));
if i(1)
    o.gr = varargin{i(1)}; varargin(i)=[];
end
...
[IComp  ,varargin] = getProp(varargin,'speciees',[]);
if isempty(IComp)
    ...
    for iComp=1:o.NCOMP
        ...
        if it==1
           ...
           % concentration
            ...
            % color bar
            hb = colorbar('peer',ax(iComp)); set(get(hb,'title'),'string',species(iComp));
