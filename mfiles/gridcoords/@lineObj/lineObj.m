...
% TO 130818
    ...
    methods
        function o=lineObj(varargin)
            ...
            isLineObj = strcmp(class(o),'lineObj'); % else area2Obj
            ...
            for io = numel(o):-1:1
                ...
                switch o(io).type
                    ...
                    case {'DRN','RIV'}
                        % use preferred columnin given order for z
        ...
        function hdl = plot(o,varargin)
            ...
            for io=1:numel(o)
                if isempty(crit)
                    ...
                    % if criterium is used for coloring object
                    if ~isempty(o(io).UserData.(['Q',o(io).type])) 
                        % color according to mean value of Q
                        iClr = ceil(interp1([crit Inf],1:numel(crit)+1,mean(sum(o(io).UserData.(['Q',o(io).type])))))+1;
                        if isnan(iClr)
                            iClr=1;
                        end
                ...
                hdl(io) = plot(ax,[o(io).P.xm],[o(io).P.ym],args{:});
        ...
        function plot3(o,varargin)
            ...
            for io=1:numel(o)
                ...
                if strcmp(class(o(io)),'area2Obj') % close the polygon
                    vm = [vm; vm(1)]; %#ok
                end
