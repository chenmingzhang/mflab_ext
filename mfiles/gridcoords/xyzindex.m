...
varargin = cellfun(@squeeze,varargin,'UniformOutput',false);

[gr,varargin] = getType(varargin,'gridObj',[]);

V = cellfun(@isvector,varargin); % a vector or a scalar (means not more than 1 column)
D = cellfun(@isnumeric,varargin); % double means it can be a multicolumn input
...
if ~isempty(gr)  % varargin contains a grid
    Nx = gr.Nx;
    Ny = gr.Ny;    
    switch sum(D)  % number of non-grid inputs
        case 1
    ...
    switch sum(D) % number of numeric arguments
        case 2,
            if all(V)
                ...
                error('invalid input combination, two args varargin must be {xY,xGr} or {xW, gr}');
            end
        case 3 
            if     ~V(1) % xyzIndex([xW yW],xGr,yGr)
               ...
               Ny = length(varargin{3})-1;
                ...
                error('%s: Don''t understand input combination',mfilename);
            end
        case 4
            if all(V)  % xyzIndex(xW,yW,xGr,yGr) 
            ...
            elseif ~V(1) % xyzIndex([xW yW zW],xGr yGr zGr)
