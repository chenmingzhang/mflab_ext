function plotXZSection(gr,varargin)
% gr.plotXZSection
% modfied from plotXS
% at the moment it is working when the soil is transverse uniform
...
for iz = 1:size(gr.Z,3)
    ...
    plot(ax,gr.xGr,gr.ZGR(1,:,iz),lineSpec,varargin{:});
end

for ix=1:gr.Nx
%    plot(ax,xGr([ix+0 ix+0]),zxs([1 end],ix),lineSpec,varargin{:});
%    plot(ax,xGr([ix+1 ix+1]),zxs([1 end],ix),lineSpec,varargin{:});
    plot(ax,squeeze(gr.XGR(1,ix,:)),squeeze(gr.ZGR(1,ix,:)),lineSpec,varargin{:});
end
