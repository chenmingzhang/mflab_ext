...
[hlines,  varargin] = getProp(varargin,{'lines','hlines'},[]);
[vlines,  varargin] = getProp(varargin,{'lines','vlines'},[]);

% should we plot hlines?
if isempty(hlines)
    vararg = [vararg, {'hlines','off','edgealpha',0.25}];
else
    vararg = [vararg, {'hlines',hlines}];
end

% should we plot vlines?
if ~isempty(vlines)
